	...
	public static PGPSecretKey readSecretKey(InputStream in,
		...
		if (data.length == 0) {
			...
			throw new IOException(
					MessageFormat.format("secretKeyTooShort {0}",
		...
		try (ByteArrayOutputStream out = new ByteArrayOutputStream(
			...
			while (pos < humanForm.length) {
				...
				if (b == '(' || b == ')') {
					...
					throw new StreamCorruptedException(
							MessageFormat.format("sexprUnhandled {0}", Integer.toHexString(b & 0xFF)));
				}
			}
		...
		while (i < to) {
			...
			case '3':
				if (i + 2 >= to || !isOctal(in[i]) || !isOctal(in[i + 1])
						|| !isOctal(in[i + 2])) {
					throw new StreamCorruptedException("sexprStringInvalidOctalEscape");
				}
				out[j++] = (byte) (((((in[i] - '0') << 3)
						| (in[i + 1] - '0')) << 3) | (in[i + 2] - '0'));
				i += 3;
