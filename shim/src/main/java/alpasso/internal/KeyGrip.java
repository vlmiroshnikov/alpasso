...
public final class KeyGrip {
	...
	public static byte[] getKeyGrip(PGPPublicKey publicKey)
		...
		case PublicKeyAlgorithmTags.EDDSA:
			...
			if (params == null) {
				throw new PGPException(MessageFormat.format("unknownCurve {0}", curveOID.getId()));
			}
			...
			if (p == null) {
				// Don't know...
				throw new PGPException(MessageFormat.format("unknownCurveParameters {0}", curveOID.getId()));
			}
		...
		default:
			throw new PGPException(
					MessageFormat.format("unknownKeyType {0}",
		...
		case 0x04:
			if (data.length != 65) {
				throw new PGPException(MessageFormat.format(
						"corrupt25519Key {0}", Hex.toHexString(data)));
			}
			// Uncompressed: should not occur with ed25519 or curve25519
			throw new PGPException(MessageFormat.format(
					"uncompressed25519Key {0}", Hex.toHexString(data)));
		case 0x40:
			if (data.length != 33) {
				throw new PGPException(MessageFormat.format(
						"corrupt25519Key {0}", Hex.toHexString(data)));
